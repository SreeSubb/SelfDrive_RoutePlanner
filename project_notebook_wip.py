...
def get_current_node(self):
    ...
    if len(self.openSet) != 0:
        ...
        for node in self.openSet:
            fval[node] = self.calculate_fscore(node)
        minVal = min(fval.values())
        lowFnode = [key for key in fval if fval[key] == minVal]
    return lowFnode
...
def distance(self, node_1, node_2):
    ...
    a = self.map.intersections[node_1]
    b = self.map.intersections[node_2]
...
def get_tentative_gScore(self, current, neighbor):
    ...
    gCurrent = get_gScore(self, current)
    distNeighbor = distance(self, current, neighbor)
    return gCurrent + distNeighbor
